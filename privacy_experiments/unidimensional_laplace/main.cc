...
void run_experiment(int ndata, int repnum) {
  ...
  for (int k = 0; k < epsilons.size(); k++) {
    std::cout << "epsilon: " << epsilons[k] << std::endl;
    ...
    bayesmix::write_matrix_to_file(
      sanitized_data, OUT_DIR + "_ndata_" + std::to_string(ndata) + "_alpha_" +
            std::to_string(alpha) + +"_rep_" + std::to_string(repnum) + "_" + "sanitized_data.csv");
